...
//The visit function checks if x is discovered yet and 
//sets it's parent to p(x). Then  visits every adjacent vertex.
//Takes care of discover/finish times and setting color to gray/black.
...
//The DFS Algorithm, stores verticies into S in DESCENDING order
//Creates a List "stack" which stores the verticies after they have been finished
//Also utilizies a variable time which increments each time a vertex is visited and finished.
...
//Returns the transpose of G, essentially reversing the direction of each edge
//of a Graph and returning that.
...
//returns a new Graph, with the same adjacency Lists as G.
Graph copyGraph(Graph G){
	Graph gCopy = newGraph( getOrder(G) );
	for (int i = 1; i < getOrder(G) + 1; i++){
		moveFront(G->adj[i]);
		for (int j = 0; j < length(G->adj[i]); j++){
			append(gCopy->adj[i],get(G->adj[i]));
			moveNext(G->adj[i]);
		}
	}
